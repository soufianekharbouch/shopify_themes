...
// 	boostPFSFilterConfig.general.separateRefineByFromFilter = jQ('.boost-pfs-filter-tree-h').length > 0 ? true : false;
	...
	/**
	 * This method is used to add an Accentuate Custom Fields product label if it set.
	 * Used on Collection pages.
	 *
	 * @param handle is a product url-key
	 * @param id to add unique id for each product__label container.
	 * @returns {string}
	 */
	function addProductCustomLabel(handle, id) {
		jQuery.ajax({
			type: "GET",
			url: "/products/" + handle + "?view=accentuate-product-label.json"
		})
			.done(function (label) {
				if (label.trim() !== '') {
					jQuery('#label-'+ id).text(label);
				}
			});

		return '<div id="label-' + id + '" class="product__label"></div>';
	}

	/**
	 * This method is replaced product title with Accentuate Custom Fields if it set.
	 * Used on Collection pages.
	 *
	 * @param handle is a product handle (url-key)
	 * @param id is product.id to add unique id for each title containers.
	 */
	function replaceProductCustomTitle(handle, id) {
		jQuery.ajax({
			type: "GET",
			url: "/products/" + handle + "?view=accentuate-product-title.json"
		})
			.done(function (title) {
				if (title.trim() !== '' && title.length > 1) {
					jQuery('#title-' + id).text(title);
				}
			});
	}
	...
	/**
	 * This method is replaced product Short Description with Accentuate Custom Fields if it set.
	 * Used on Collection pages.
	 *
	 * @param handle is a product handle (url-key)
	 * @param id is product.id to add unique id for each short description containers.
	 */
	function replaceProductCustomShortDesc(handle, id) {

		let customShortDesc = null;
		jQuery.ajax({
			type: "GET",
			url: "/products/" + handle + "?view=accentuate-product-short-desc.json"
		})
			.done(function (desc) {
				if (desc.trim() !== '' && desc.length > 1) {
					jQuery('#short-desc-' + id).text(desc);
					var startsReviews=$('<div class="YotoStartGrid"></div>')
					startsReviews.html($('#yotpoStart-' + id).html())
					startsReviews.insertAfter($(('#short-desc-' + id)))
				}
			});
	}
	...
	ProductGridItem.prototype.compileTemplate = function(data) {
		...
		console.log("CHECK HIDDEN TAG FOR "+data.title)
		if(data.tags.includes("Hidden")) {
			console.log("Tag Hidden Exist")
			return "";
		}else{
			console.log("Tag Hidden Doesnt Exist")
		}
		...
		var saleClass = onSale ? boostPFSTemplate.saleClass : '';
      	...
      	// add unique id for title and short description selectors
		itemHtml = itemHtml.replace(/{{titleUniqueId}}/g, 'title-' + data.id);
		itemHtml = itemHtml.replace(/{{shortDescUniqueId}}/g, 'short-desc-' + data.id);
		replaceProductCustomTitle(data.handle, data.id);
		replaceProductCustomShortDesc(data.handle, data.id);
        ...
        itemHtml = itemHtml.replace(/{{itemSizeSwatches}}/g, buildProductOptionSwatches(data, boostPFSFilterConfig.boostProductOptionSwatch.size.swatchName, boostPFSFilterConfig.boostProductOptionSwatch.size.optionName, boostPFSFilterConfig.boostProductOptionSwatch.size.swatchDisplay, boostPFSFilterConfig.boostProductOptionSwatch.size.swatchType ));
      	...
      	itemHtml = itemHtml.replace(/{{productLabel}}/g, addProductCustomLabel(data.handle, data.id));
      
      	itemHtml = itemHtml.replace(/{{variantInputs}}/g, buildVariantInputs(data));
        itemHtml = itemHtml.replace(/{{qtyInput}}/g, buildQtyInput(data));
        itemHtml = itemHtml.replace(/{{buyButton}}/g, buildBuyBtn(data, Utils.buildProductItemUrl(data), Utils.getProductMetafield(data, 'accentuate', 'is_subscription_product') ? true : false));
        itemHtml = itemHtml.replace(/{{variantsCount}}/g, buildVariantsCount(data));
      
		/**
		 * TODO: there is no default value set. Is this logic correct in its original form?
		 * TODO: the current state is commented out as redundant.
		 * I leave replacing the variable with an empty string as the default action for now.
		 * @author Andrii Borhoiakov.
		 * @ticket BEEK-1084
		 */
		// current state
		// itemHtml = itemHtml.replace(/{{short_description}}/g, Utils.getProductMetafield(data, 'accentuate', 'short_description') ? Utils.getProductMetafield(data, 'accentuate', 'short_description') : '');

		// replacement with an empty string as the default action
		itemHtml = itemHtml.replace(/{{short_description}}/g,  '');
	...
	function buildImages(data) {
		...
		html += '<div class="responsive-image__wrapper">';
	...
	function buildPrice(data) {
		...
		for (collection of data.collections){
			...
			if(cross_out_price_collection_handle.includes(handle) ){
                  is_tagged=true; 
				  indexPrice=cross_out_price_collection_handle.indexOf(handle);
				  discountValue=Number(cross_out_value[indexPrice]);
		...
		if (boostPFSThemeConfig.custom.hasOwnProperty('show_price') &&
			...
			if (onSale || is_tagged) {
			 	if(is_tagged){
					let discountPrice=Number(data.price_max-data.price_max*discountValue);
					html += '<strike><span class="boost-pfs-filter-product-item-sale-price">' + Utils.formatMoney(data.price_max) + '</span></strike>  ';
					html += '  <span style="color:dark gray;" >' + Utils.formatMoney(discountPrice) + '</span> ';
				}else{
					html += '<span class="boost-pfs-filter-product-item-sale-price">' + Utils.formatMoney(data.price_max) + '</span>';
    ...
    function buildBuyBtn(data, url, isSub) {
      ...
      if (boostPFSThemeConfig.label_basic.sold_out) {
        soldOutLabel = boostPFSThemeConfig.label_basic.sold_out;
      }
      ...
      /* #boost-86411: Fix showing sold out for available variants */
      // if($('#selected_or_first_available_variant_'+data.handle).val()==="true")
      if(firstVariant.available)
        return '<button type="submit" name="add" class="add__to" id="buyBtn'+ firstVariant.id +'" data-add-to-cart>' +
          '<span data-add-to-cart-text>' +
          'Add To Cart' +
          '</span>' +
          '</button>';
      else
        return '<button type="submit" name="add" class="add__to" id="buyBtn'+ firstVariant.id +'" data-add-to-cart disabled="disabled"><span class="sold_out_product">' + soldOutLabel + '</span></button>';
...
// 	};
    ...
    FilterMobileButton.prototype.afterRender = function() {
		console.log(this);
      	if (this.$element.find('.arrow-wrap').length == 0) {
          	this.$element.append('<div class="arrow-wrap"><svg width="12" height="6" viewBox="0 0 12 6"><polygon fill="#4A5761" points="0 0 5.394 5.394 0 10.787" transform="rotate(90 5.5 6)"></polygon></svg></div>');
        }
    }
			...
			if (boostPFSThemeConfig.custom.hasOwnProperty('active_image_swap') &&
				...
				jQ(".boost-pfs-filter-product-item").each(function() {
					...
					if (typeof imgSelector.data('img-flip-src') != 'undefined' &&
						...
						productItemSelector.find('.boost-pfs-filter-product-item-image-link .responsive-image__wrapper').append(html);
					}
