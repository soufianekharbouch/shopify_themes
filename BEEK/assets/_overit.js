...
function fireHolidayNav($){
  $('.holiday-sidebar .nav-item > a').on( 'click', function(evt) {
  	evt.preventDefault();
    var $subnav = $( this ).siblings('.subnav');
    var $isHidden = $subnav.hasClass('hide');
    //$('.holiday-sidebar .subnav').addClass('hide');
    if( $isHidden ){
      $subnav.removeClass('hide');
      $( this ).find('i.fa-angle-right').toggleClass('fa-angle-right fa-angle-up');
    }else{
      $subnav.addClass('hide');
      $( this ).find('i.fa-angle-up').toggleClass('fa-angle-up fa-angle-right');
    }
  });
}	// end fireHolidayNav()
...
function adjustHeaderImage($){
	$('.holiday-header.section-header--image').css({ 'margin-top': $('.site-header').height() });
}	// end adjustHeaderImage()


function getPageAndExpandNav($){
  	var $pageID = $('body').attr('id');
  	// find the subnav with this item
	$(".nav-subitem."+$pageID).parent().removeClass("hide");
	$(".nav-subitem."+$pageID+" a").css({ 'color': '#874F49' });
}	// end getPageAndExpandNav($)


function updateMainLogoURL($){
	var isHoliday = $('body').hasClass('template-collection-holiday-favorites');
	if( isHoliday ){
		// update the URL of the main logo
      	//$('header .logo-link').attr('href', '/collections/holiday-favorites');
	}
}	// end updateMainLogoURL()
...
function autoResizeIframe($){
	var frameH = $('#quizIframe').contents().height();
	console.log( 'frameH', frameH );
	$('#quizIframe').height( frameH );
...
// 	iFrameResize({log:true}, '#quizIframe');
	autoResizeIframe($);
